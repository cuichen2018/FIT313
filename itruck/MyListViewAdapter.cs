

using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

using Android.App;
using Android.Content;
using Android.OS;
using Android.Runtime;
using Android.Views;
using Android.Widget;

namespace itruck
{
    [Activity(Label = "MyListViewAdapter")]
    public class MyListViewAdapter : BaseAdapter<string>
    {
        private List<string> trucks;
        private Context mContext;

        public MyListViewAdapter(Context context, List<string> items)
        {
            trucks = items;
            mContext = context;
        }
       
        public override int Count
        {
            get { return trucks.Count; }
        }

        public override long GetItemId(int position)
        {
            return position;
        }

        public override string this[int position]
        {
            get { return trucks[position]; }
        }

        public override View GetView(int position, View convertView, ViewGroup parent)
        {
            View row = convertView;

            if(row == null)
            {
                row = LayoutInflater.From(mContext).Inflate(Resource.Layout.TruckListView, null, false);
            }
            TextView textView = row.FindViewById<TextView>(Resource.Id.name);
            textView.Text = trucks[position];

            return row;
        }
    }
}
